    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <x-splade-table :for="$pribors">
                    @cell('status', $staff)
                        @if($staff->status == 0)
                            <span class="text-orange-500 font-semibod">Не используется</span>
                        ...
                        @elseif ($staff->status == 4)
                            <span class="text-red-500 font-semibod">Не исправлен</span>
                        @endif
                    @endcell
                    @cell('action', $pribor)
